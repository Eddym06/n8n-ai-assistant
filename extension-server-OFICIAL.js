...
// 🔍 IMPORTACIÓN DEL AGENTE DE VALIDACIÓN INTELIGENTE
...
// 🧠 IMPORTACIÓN DEL AGENTE ULTRA INTELIGENTE DE FALLBACK V2.0
...
// 🎯 INTEGRACIÓN DEL SISTEMA DE POSICIONAMIENTO INTELIGENTE V2.0 Y V4 AI ENHANCED
let IntelligentPositioningAgentV2 = null;
let IntelligentPositioningAgentV4AIEnhanced = null;

// 🤖 INTEGRACIÓN DEL AGENTE DE CONFIGURACIÓN DE NODOS AVANZADO V3 - TEMPORALMENTE DESHABILITADO
let IntelligentNodeConfigAgentV3 = null; // DESHABILITADO PARA TESTING

// 🚀 AGENTES ENTERPRISE V4.0 - SISTEMA PROFESIONAL COMPLETO
...
async function initializeV3Agents() {
  ...
  if (!IntelligentPositioningAgentV2) {
    try {
      console.log('📦 Cargando IntelligentPositioningAgentV2...');
      const module = await import('./Agentes y sistemas Antiguos/intelligent-positioning-agent-v2.js');
      IntelligentPositioningAgentV2 = module.default;
      console.log('✅ IntelligentPositioningAgentV2 cargado exitosamente');
    } catch (error) {
      console.log('🔄 IntelligentPositioningAgentV2 no disponible:', error.message);
      IntelligentPositioningAgentV2 = null;
    }
  }
  
  if (!IntelligentPositioningAgentV4AIEnhanced) {
    try {
      console.log('📦 Cargando IntelligentPositioningAgentV4AIEnhanced...');
      ...
      console.log('✅ IntelligentPositioningAgentV4AIEnhanced cargado exitosamente');
    } catch (error) {
      console.log('🔄 IntelligentPositioningAgentV4AIEnhanced no disponible:', error.message);
      IntelligentPositioningAgentV4AIEnhanced = null;
    }
  }
  ...
  if (!IntelligentNodeConfigAgentV3) {
    try {
      ...
      const module = await import('./intelligent-node-config-agent-v3-FIXED.js');
  ...
  */
  IntelligentNodeConfigAgentV3 = null; // FORZADO A NULL PARA TESTING
  
  console.log('🎯 Estado final agentes V3.0 Ultra:');
  console.log('   🔧 FlowCoherenceAgentV2:', FlowCoherenceAgentV2 ? '✅' : '❌');
  console.log('   🎯 IntelligentPositioningAgentV2:', IntelligentPositioningAgentV2 ? '✅' : '❌');
  console.log('   🎯 IntelligentPositioningAgentV4AIEnhanced:', IntelligentPositioningAgentV4AIEnhanced ? '✅' : '❌');
  console.log('   🤖 IntelligentNodeConfigAgentV3:', IntelligentNodeConfigAgentV3 ? '✅' : '❌');
}

// 🚀 FUNCIÓN DE INICIALIZACIÓN PARA AGENTES ENTERPRISE V4.0
async function initializeEnterpriseAgentsV4() {
  console.log('🚀 Inicializando SISTEMA ENTERPRISE V4.0...');
  
  // Inicializar agente de configuración de nodos enhanced
  if (!EnterpriseAgentsV4.nodeConfig) {
    try {
      console.log('📦 Cargando IntelligentNodeConfigAgentEnhanced...');
      EnterpriseAgentsV4.nodeConfig = new IntelligentNodeConfigAgentEnhanced();
      console.log('✅ IntelligentNodeConfigAgentEnhanced inicializado');
    } catch (error) {
      console.log('❌ Error inicializando IntelligentNodeConfigAgentEnhanced:', error.message);
      EnterpriseAgentsV4.nodeConfig = null;
    }
  }
  
  // Inicializar agente de posicionamiento enhanced
  if (!EnterpriseAgentsV4.positioning) {
    try {
      console.log('📦 Cargando IntelligentPositioningAgentEnhanced...');
      EnterpriseAgentsV4.positioning = new IntelligentPositioningAgentEnhanced();
      console.log('✅ IntelligentPositioningAgentEnhanced inicializado');
    } catch (error) {
      console.log('❌ Error inicializando IntelligentPositioningAgentEnhanced:', error.message);
      EnterpriseAgentsV4.positioning = null;
    }
  }
  
  // Inicializar agente de integración MCP automática
  if (!EnterpriseAgentsV4.mcpIntegration) {
    try {
      console.log('📦 Cargando AutomaticMCPIntegrationAgent...');
      EnterpriseAgentsV4.mcpIntegration = new AutomaticMCPIntegrationAgent();
      console.log('✅ AutomaticMCPIntegrationAgent inicializado');
    } catch (error) {
      console.log('❌ Error inicializando AutomaticMCPIntegrationAgent:', error.message);
      EnterpriseAgentsV4.mcpIntegration = null;
    }
  }
  
  // Inicializar agente de lógica condicional avanzada
  if (!EnterpriseAgentsV4.conditionalLogic) {
    try {
      console.log('📦 Cargando AdvancedConditionalLogicAgent...');
      EnterpriseAgentsV4.conditionalLogic = new AdvancedConditionalLogicAgent();
      console.log('✅ AdvancedConditionalLogicAgent inicializado');
    } catch (error) {
      console.log('❌ Error inicializando AdvancedConditionalLogicAgent:', error.message);
      EnterpriseAgentsV4.conditionalLogic = null;
    }
  }
  
  // Inicializar validador conversacional de workflows
  if (!EnterpriseAgentsV4.workflowValidator) {
    try {
      console.log('📦 Cargando ConversationalWorkflowValidator...');
      EnterpriseAgentsV4.workflowValidator = new ConversationalWorkflowValidator();
      console.log('✅ ConversationalWorkflowValidator inicializado');
    } catch (error) {
      console.log('❌ Error inicializando ConversationalWorkflowValidator:', error.message);
      EnterpriseAgentsV4.workflowValidator = null;
    }
  }
  
  // Inicializar plantillas enterprise
  if (!EnterpriseAgentsV4.templates) {
    try {
      console.log('📦 Cargando EnterpriseAgentTemplates...');
      EnterpriseAgentsV4.templates = new EnterpriseAgentTemplates();
      console.log('✅ EnterpriseAgentTemplates inicializado');
    } catch (error) {
      console.log('❌ Error inicializando EnterpriseAgentTemplates:', error.message);
      EnterpriseAgentsV4.templates = null;
    }
  }
  
  console.log('🚀 Estado final AGENTES ENTERPRISE V4.0:');
  console.log('   🤖 NodeConfig Enhanced:', EnterpriseAgentsV4.nodeConfig ? '✅' : '❌');
  console.log('   🎯 Positioning Enhanced:', EnterpriseAgentsV4.positioning ? '✅' : '❌');
  console.log('   🔌 MCP Integration:', EnterpriseAgentsV4.mcpIntegration ? '✅' : '❌');
  console.log('   🧠 Conditional Logic:', EnterpriseAgentsV4.conditionalLogic ? '✅' : '❌');
  console.log('   ✅ Workflow Validator:', EnterpriseAgentsV4.workflowValidator ? '✅' : '❌');
  console.log('   📋 Enterprise Templates:', EnterpriseAgentsV4.templates ? '✅' : '❌');
  
  return EnterpriseAgentsV4;
}

// 📊 IMPORTACIÓN DEL SISTEMA DE TRACKING GLOBAL DE GEMINI
import GeminiCallTracker from './SISTEMA PRINCIPAL/gemini-call-tracker.js';

// 🎯 ENRUTADOR INTELIGENTE DE MODELOS GEMINI
// ==========================================
/**
 * Sistema de enrutamiento inteligente que maneja:
 * - Modelos PRO exclusivos para Extension Server OFICIAL
 * - Modelos FLASH para todos los agentes
 * - Rotación automática cuando se agotan cuotas
 * - Sistema de timing inteligente con pausas graduales
 * - Logging detallado de qué modelo usa cada componente
 */
class GeminiModelRouter {
  constructor() {
    this.currentServerModel = 'gemini-2.5-pro'; // Modelo base para Extension Server
    this.currentAgentModel = 'gemini-2.5-flash'; // Modelo base para Agentes
    
    // Modelos disponibles PRO (solo Extension Server)
    this.proModels = [
      'gemini-2.5-pro',
    ...
    // Modelos disponibles FLASH (solo Agentes)
    this.flashModels = [
      'gemini-2.5-flash',
    ...
    // Índices de modelos actuales
    this.currentProIndex = 0;
    this.currentFlashIndex = 0;
    
    // Sistema de timing inteligente
    this.retryDelays = [15000, 30000, 45000]; // 15s, 30s, 45s
    this.currentRetryAttempt = 0;
    
    // Contadores y estadísticas
    this.stats = {
      serverModelSwitches: 0,
      agentModelSwitches: 0,
      totalRetries: 0,
      pausesDuration: 0
    };

    console.log('🎯 GEMINI MODEL ROUTER INICIADO');
    console.log(`📊 Extension Server usará: ${this.currentServerModel}`);
    console.log(`🤖 Agentes usarán: ${this.currentAgentModel}`);
    console.log('🔄 Sistema de rotación automática activado');
  }

  /**
   * Obtener modelo actual para Extension Server OFICIAL
   */
  getServerModel() {
    console.log(`🎯 Extension Server usando: ${this.currentServerModel}`);
    return this.currentServerModel;
  }

  /**
   * Obtener modelo actual para todos los Agentes
   */
  getAgentModel() {
    console.log(`🤖 Agentes usando: ${this.currentAgentModel}`);
    return this.currentAgentModel;
  }

  /**
   * Rotar modelo PRO para Extension Server
   */
  rotateServerModel() {
    this.currentProIndex = (this.currentProIndex + 1) % this.proModels.length;
    this.currentServerModel = this.proModels[this.currentProIndex];
    this.stats.serverModelSwitches++;
    
    console.log(`🔄 Extension Server rotando a: ${this.currentServerModel}`);
    console.log(`📊 Rotaciones Extension Server: ${this.stats.serverModelSwitches}`);
    
    return this.currentServerModel;
  }

  /**
   * Rotar modelo FLASH para Agentes
   */
  rotateAgentModel() {
    this.currentFlashIndex = (this.currentFlashIndex + 1) % this.flashModels.length;
    this.currentAgentModel = this.flashModels[this.currentFlashIndex];
    this.stats.agentModelSwitches++;
    
    console.log(`🔄 Agentes rotando a: ${this.currentAgentModel}`);
    console.log(`📊 Rotaciones Agentes: ${this.stats.agentModelSwitches}`);
   ...
   * Sistema de timing inteligente con pausas graduales
   */
  async handleRetryWithPause() {
    if (this.currentRetryAttempt >= this.retryDelays.length) {
      console.log('⚠️ Máximo de intentos alcanzado, reiniciando contador');
      this.currentRetryAttempt = 0;
      return false;
    }

    const delay = this.retryDelays[this.currentRetryAttempt];
    this.stats.totalRetries++;
    this.stats.pausesDuration += delay;
    
    console.log(`⏱️ Sistema de timing: Pausa ${this.currentRetryAttempt + 1}/3 - ${delay/1000}s`);
    console.log(`📊 Total pausas: ${this.stats.totalRetries} (${this.stats.pausesDuration/1000}s acumulados)`);
  ...
  /**
   * Manejar error y decidir estrategia de recuperación
   */
  async handleError(error, isServer = true) {
    console.log(`❌ Error en ${isServer ? 'Extension Server' : 'Agentes'}:`, error.message);
    ...
    const isTrafficError = error.message.includes('traffic') ||
                          error.message.includes('busy') ||
                          error.message.includes('unavailable');

    if (isQuotaError) {
      console.log('🚫 Error de cuota detectado, rotando modelo...');
      if (isServer) {
        this.rotateServerModel();
      } else {
        this.rotateAgentModel();
      }
      return true; // Reintentar inmediatamente con nuevo modelo
    }

    if (isTrafficError) {
      console.log('🌐 Error de tráfico detectado, aplicando timing inteligente...');
      const shouldContinue = await this.handleRetryWithPause();
      if (shouldContinue) {
        // Opcionalmente también rotar modelo
        if (Math.random() > 0.5) { // 50% chance de rotar modelo además de la pausa
          if (isServer) {
            this.rotateServerModel();
          } else {
            this.rotateAgentModel();
    ...
    // Error desconocido, rotar modelo y aplicar pausa corta
    console.log('❓ Error desconocido, aplicando estrategia combinada...');
    if (isServer) {
      this.rotateServerModel();
    } else {
      this.rotateAgentModel();
    ...
    await new Promise(resolve => setTimeout(resolve, 5000)); // Pausa corta de 5s
    return true;
  }

  /**
   * Ejecutar llamada con manejo automático de errores
   */
  async executeWithRetry(operation, isServer = true, maxAttempts = 3) {
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        console.log(`🎯 Intento ${attempt}/${maxAttempts} - ${isServer ? 'Extension Server' : 'Agentes'}`);
        const result = await operation();
        
        // Reset retry counter on success
        if (attempt > 1) {
          this.currentRetryAttempt = 0;
          console.log('✅ Operación exitosa después de reintentos');
        }
        ...
        if (attempt === maxAttempts) {
          ...
          throw new Error(error.message || `Error después de ${maxAttempts} intentos: ${error.toString()}`);
        }
        
        const shouldContinue = await this.handleError(error, isServer);
        if (!shouldContinue) {
          throw new Error(error.message || `Error procesando solicitud: ${error.toString()}`);
        }
      }
    }
  }
  ...
  getStats() {
    return {
      ...
      serverModelSwitches: this.stats.serverModelSwitches,
      agentModelSwitches: this.stats.agentModelSwitches,
      totalRetries: this.stats.totalRetries,
      totalPauseTime: `${this.stats.pausesDuration/1000}s`,
      availableProModels: this.proModels,
      availableFlashModels: this.flashModels
    };
  }

  /**
   * Mostrar estado completo del enrutador
   */
  showStatus() {
    console.log('\n📊 ESTADO DEL ENRUTADOR DE MODELOS');
    console.log(''.padEnd(40, '='));
    console.log(`🎯 Extension Server: ${this.currentServerModel}`);
    console.log(`🤖 Agentes: ${this.currentAgentModel}`);
    console.log(`🔄 Rotaciones Server: ${this.stats.serverModelSwitches}`);
    console.log(`🔄 Rotaciones Agentes: ${this.stats.agentModelSwitches}`);
    console.log(`⏱️ Total reintentos: ${this.stats.totalRetries}`);
    console.log(`⏱️ Tiempo total pausas: ${this.stats.pausesDuration/1000}s`);
    console.log(''.padEnd(40, '='));
  }
}

// 🔧 INTEGRACIÓN DEL AUTOCORRECTOR INTELIGENTE
import AutocorrectorFlujos from './SISTEMA PRINCIPAL/Herramienta-Autocorrector.js';

// 🔧 INTEGRACIÓN DEL CORRECTOR DE NOMBRES INTELIGENTE
import IntelligentNameCorrector from './SISTEMA PRINCIPAL/intelligent-name-corrector.js';

// 🎯 INTEGRACIÓN DE AGENTES INDEPENDIENTES
import PromptEnhancementAgent from './SISTEMA PRINCIPAL/prompt-enhancement-agent.js';
import WorkflowVectorSearchEngine from './SISTEMA PRINCIPAL/workflow-vector-search-engine-esm.js';
import SemanticMemoryAgent from './SISTEMA PRINCIPAL/semantic-memory-agent.js';

// 🚀 INTEGRACIÓN DE NUEVOS AGENTES ENTERPRISE V4.0
import IntelligentNodeConfigAgentEnhanced from './SISTEMA PRINCIPAL/intelligent-node-config-agent-v3-enhanced.js';
import IntelligentPositioningAgentEnhanced from './SISTEMA PRINCIPAL/intelligent-positioning-agent-v4-ai-enhanced.js';
import AutomaticMCPIntegrationAgent from './SISTEMA PRINCIPAL/automatic-mcp-integration-agent.js';
import AdvancedConditionalLogicAgent from './SISTEMA PRINCIPAL/advanced-conditional-logic-agent.js';
import ConversationalWorkflowValidator from './SISTEMA PRINCIPAL/conversational-workflow-validator.js';
import EnterpriseAgentTemplates from './SISTEMA PRINCIPAL/enterprise-agent-templates.js';
  ...
  async requestGeminiCompletion(truncatedJSON, truncationPoint, options, originalPrompt) {
    try {
      if (!this.model) {
        ...
        // Usar modelo del enrutador para Extension Server (Pro models)
        const serverModel = this.assistant?.modelRouter?.getServerModel() || 'gemini-2.5-pro';
        this.model = genAI.getGenerativeModel({ model: serverModel });
        console.log(`🔧 JSONRepairAgent usando modelo del Extension Server: ${serverModel}`);
      }
...
async function main() {
  ...
  // 🚀 Inicializar agentes enterprise V4.0
  console.log('🔧 Inicializando sistema enterprise...');
  await initializeV3Agents();
  await initializeEnterpriseAgentsV4();
  ...
  try {
    if (isTestExtender) {
      ...
      const result = await assistant.processUserPromptV3(inputPrompt);
      if (result.success) {
...
class N8nAIAssistant {
  constructor() {
    // 🎯 INICIALIZAR ENRUTADOR DE MODELOS GEMINI
    this.modelRouter = new GeminiModelRouter();
    
    this.searchAgent = new WorkflowVectorSearchEngine();
    this.promptAgent = new PromptEnhancementAgent();
    this.semanticMemory = new SemanticMemoryAgent();
    this.lastGeneratedFilePath = null;

    // � CONECTAR AGENTES AL ENRUTADOR DE MODELOS
    // Los agentes solo pueden usar modelos Flash, nunca Pro
    if (this.promptAgent && typeof this.promptAgent.setModelRouter === 'function') {
      this.promptAgent.setModelRouter(this.modelRouter);
    }
    if (this.semanticMemory && typeof this.semanticMemory.setModelRouter === 'function') {
      this.semanticMemory.setModelRouter(this.modelRouter);
    }
    console.log('🔗 Agentes conectados al enrutador de modelos');
    ...
    // 🧠 INTEGRACIÓN DEL AGENTE ULTRA INTELIGENTE DE FALLBACK V2.0
    this.ultraIntelligentFallback = new UltraIntelligentFallbackAgent();
      ...
      switch (node.type) {
        ...
        case 'n8n-nodes-base.httpRequest':
          if (!originalParams.url) {
            if (isMarketing && node.name.includes('AI_Classify')) {
              ...
              node.parameters.body = JSON.stringify({
                ...
                messages: [{
                  role: 'user',
                  content: 'Clasifica este lead: {{ $json.message }}'
                }]
              });
            } else if (node.name.includes('CRM')) {
              node.parameters.url = 'https://api.hubspot.com/crm/v3/objects/contacts';
              node.parameters.method = 'POST';
              node.parameters.headers = {
...
return [{
  json: {
    ...
    emergencyProcessed: true
        ...
        case 'n8n-nodes-base.googleSheets':
          if (!originalParams.documentId) {
            node.parameters.documentId = '1AbCdE2fGhI3jKlM4nOpQ5rStU6vWxY7zA8bC9dE0fGh';
            paramsUpdated = true;
          }
          if (!originalParams.operation) {
            node.parameters.operation = 'append';
            paramsUpdated = true;
          }
          if (!originalParams.range) {
            node.parameters.range = 'Sheet1!A:Z';
            paramsUpdated = true;
          }
          if (!originalParams.columns && node.parameters.operation === 'append') {
            node.parameters.columns = [
              { column: 'A', value: '{{ $json.timestamp }}' },
              { column: 'B', value: '{{ $json.name }}' },
              { column: 'C', value: '{{ $json.email }}' },
  ...
  // GENERADOR INTELIGENTE DE WORKFLOW FALLBACK V2.0 - CON WORKFLOWS DE REFERENCIA
  async generateIntelligentFallbackWorkflow(prompt) {
    ...
    console.log(`🤖 Generando workflow inteligente MEJORADO con MÍNIMO ${minNodes} NODOS...`);
    console.log(`   📊 Complejidad detectada: ${complexity}`);
    
    // 🔍 NUEVA FUNCIONALIDAD: Buscar workflows de referencia similares
    let referenceWorkflows = [];
    let selectedReference = null;
    
    try {
      console.log('🔍 Buscando workflows de referencia similares...');
      const similarWorkflows = await this.searchAgent.searchSimilarWorkflows(prompt);
      const curatedExamples = this.searchAgent.getCuratedExamples();
      referenceWorkflows = [...similarWorkflows, ...curatedExamples];
      
      console.log(`📚 Encontrados ${referenceWorkflows.length} workflows de referencia`);
      
      // Seleccionar el mejor workflow de referencia
      selectedReference = this.selectBestReferenceWorkflow(referenceWorkflows, prompt, complexity);
      
      if (selectedReference) {
        console.log(`✅ Workflow de referencia seleccionado: ${selectedReference.name || 'workflow-base'}`);
        console.log(`   📊 Nodos en referencia: ${selectedReference.workflow?.nodes?.length || 'N/A'}`);
        console.log(`   🎯 Score de similitud: ${selectedReference.similarityScore || 'N/A'}`);
      } else {
        console.log('⚠️ No se encontró workflow de referencia adecuado, usando lógica hardcodeada');
      }
    } catch (error) {
      console.warn('⚠️ Error buscando workflows de referencia:', error.message);
      console.log('🔄 Continuando con lógica hardcodeada fallback...');
    }
    ...
    // 🚀 GENERAR NODOS USANDO WORKFLOW DE REFERENCIA O LÓGICA HARDCODEADA
    let nodes = [];
    let connections = {};
    let nodeCount = 0;
    
    if (selectedReference && selectedReference.workflow) {
      console.log('🎯 Construyendo workflow basado en referencia seleccionada...');
      
      // Usar el workflow de referencia como base y adaptarlo al prompt
      const baseWorkflow = this.adaptReferenceWorkflow(selectedReference.workflow, prompt, entities, complexity);
      nodes = baseWorkflow.nodes;
      connections = baseWorkflow.connections;
      nodeCount = nodes.length;
      
      console.log(`✅ Workflow base adaptado: ${nodeCount} nodos generados desde referencia`);
      
      // Complementar con nodos adicionales si es necesario para alcanzar minNodes
      if (nodeCount < minNodes) {
        console.log(`🔧 Complementando con ${minNodes - nodeCount} nodos adicionales...`);
        const additionalNodes = this.generateComplementaryNodes(nodes, entities, workflowType, minNodes - nodeCount);
        nodes.push(...additionalNodes);
        
        // Actualizar conexiones para integrar los nuevos nodos
        connections = this.integrateAdditionalNodes(connections, nodes);
        console.log(`✅ Workflow complementado: ${nodes.length} nodos totales`);
      }
      
    } else {
      console.log('🔧 Construyendo workflow con lógica hardcodeada...');
      
      // LÓGICA ORIGINAL HARDCODEADA
  ...
  // 🔄 ADAPTADOR DE WORKFLOW DE REFERENCIA
  adaptReferenceWorkflow(referenceWorkflow, prompt, entities, complexity) {
    console.log('🔄 Adaptando workflow de referencia al prompt específico...');
    
    if (!referenceWorkflow || !referenceWorkflow.nodes) {
      console.warn('⚠️ Workflow de referencia inválido');
      return { nodes: [], connections: {} };
    }
    
    // Crear una copia profunda del workflow de referencia
    const adaptedNodes = JSON.parse(JSON.stringify(referenceWorkflow.nodes));
    const adaptedConnections = JSON.parse(JSON.stringify(referenceWorkflow.connections || {}));
    
    // Personalizar nombres de nodos según el prompt
    const promptKeywords = this.extractKeywordsFromPrompt(prompt);
    
    adaptedNodes.forEach((node, index) => {
      // Generar nuevo ID único
      node.id = `adapted-${index}-${Date.now()}`;
      
      // Personalizar nombre del nodo
      node.name = this.personalizeNodeName(node.name, node.type, promptKeywords, entities);
      
      // Actualizar posición para evitar solapamiento
      if (node.position) {
        node.position[0] = 200 + (index * 300);
        node.position[1] = 300 + Math.floor(index / 5) * 200;
      }
      
      // Personalizar parámetros básicos si es posible
      if (node.parameters && promptKeywords.length > 0) {
        node.parameters = this.personalizeNodeParameters(node.parameters, node.type, promptKeywords);
      }
    });
    
    console.log(`✅ Workflow adaptado: ${adaptedNodes.length} nodos personalizados`);
  ...
  // 🏷️ PERSONALIZADOR DE NOMBRES DE NODOS
  personalizeNodeName(originalName, nodeType, promptKeywords, entities) {
    // Mantener el nombre original si no hay keywords relevantes
    if (!promptKeywords.length) return originalName;
    
    // Mapear tipos de nodos a contextos específicos
    const nodeTypeMap = {
      'webhook': 'Webhook',
      'if': 'Condición',
      'function': 'Procesador',
    ...
    const baseType = nodeType?.replace('n8n-nodes-base.', '') || 'Nodo';
    const contextualName = nodeTypeMap[baseType] || baseType;
    
    // Usar el primer keyword relevante para personalizar
    const relevantKeyword = promptKeywords.find(kw => 
      ['ventas', 'marketing', 'soporte', 'inventario', 'facturación'].includes(kw)
    ) || promptKeywords[0];
    
    if (relevantKeyword) {
      return `${contextualName} ${relevantKeyword.charAt(0).toUpperCase() + relevantKeyword.slice(1)}`;
    }
    
    return originalName;
  }

  // ⚙️ PERSONALIZADOR DE PARÁMETROS DE NODOS
  personalizeNodeParameters(parameters, nodeType, promptKeywords) {
    // Clonar parámetros para no modificar el original
    const personalizedParams = { ...parameters };
    
    // Personalizar según el tipo de nodo
    const baseType = nodeType?.replace('n8n-nodes-base.', '');
    
    switch (baseType) {
      case 'webhook':
        if (promptKeywords.includes('ventas')) {
          personalizedParams.path = 'ventas-webhook';
        } else if (promptKeywords.includes('soporte')) {
          personalizedParams.path = 'soporte-webhook';
        }
        break;
        
      case 'emailSend':
        if (promptKeywords.includes('bienvenida')) {
          personalizedParams.subject = 'Bienvenida - Proceso automatizado';
        } else if (promptKeywords.includes('confirmación')) {
          personalizedParams.subject = 'Confirmación - Proceso completado';
        }
        break;
        
      case 'set':
        // Agregar contexto específico al set de datos
        if (promptKeywords.includes('cliente')) {
          personalizedParams.values = personalizedParams.values || {};
        }
        break;
    }
  ...
  // 🔧 GENERADOR DE NODOS COMPLEMENTARIOS
  generateComplementaryNodes(existingNodes, entities, workflowType, count) {
    console.log(`🔧 Generando ${count} nodos complementarios...`);
    
    const complementaryNodes = [];
    let nodeIndex = existingNodes.length;
    
    // Determinar qué tipos de nodos faltan
    const existingTypes = new Set(existingNodes.map(n => n.type?.replace('n8n-nodes-base.', '')));
    
    // Lista de nodos complementarios comunes por prioridad
    const complementaryTypes = [
      { type: 'if', creator: () => this.createConditionNode(nodeIndex++) },
      { type: 'set', creator: () => this.createValidationNode(nodeIndex++) },
      { type: 'httpRequest', creator: () => this.createEnrichmentNode(nodeIndex++) },
      { type: 'emailSend', creator: () => this.createNotificationNode(nodeIndex++) },
      { type: 'function', creator: () => this.createDataCleaningNode(nodeIndex++) }
    ];
    
    // Agregar nodos que no existen hasta completar el count
    for (let i = 0; i < count && i < complementaryTypes.length; i++) {
      const complement = complementaryTypes[i];
      if (!existingTypes.has(complement.type)) {
        try {
          const newNode = complement.creator();
          complementaryNodes.push(newNode);
        } catch (error) {
          console.warn(`⚠️ Error creando nodo complementario ${complement.type}:`, error.message);
        }
      }
    }
    
    console.log(`✅ ${complementaryNodes.length} nodos complementarios generados`);
    return complementaryNodes;
  }

  // 🔗 INTEGRADOR DE NODOS ADICIONALES
  integrateAdditionalNodes(existingConnections, allNodes) {
    console.log('🔗 Integrando conexiones para nodos adicionales...');
    
    // Clonar conexiones existentes
    const integratedConnections = { ...existingConnections };
    
    // Si hay nodos sin conexiones, conectarlos secuencialmente
    const connectedNodes = new Set(Object.keys(existingConnections));
    const allNodeNames = allNodes.map(n => n.name);
    
    allNodeNames.forEach((nodeName, index) => {
      if (!connectedNodes.has(nodeName) && index > 0) {
        // Conectar al nodo anterior disponible
        const previousNode = allNodeNames[index - 1];
        if (previousNode && !integratedConnections[previousNode]) {
          integratedConnections[previousNode] = {
    ...
    console.log(`✅ Conexiones integradas para ${Object.keys(integratedConnections).length} nodos`);
  ...
  // 🎯 SELECTOR INTELIGENTE DE WORKFLOW DE REFERENCIA
  selectBestReferenceWorkflow(referenceWorkflows, prompt, complexity) {
    if (!referenceWorkflows || referenceWorkflows.length === 0) {
      return null;
    }
    
    console.log('🎯 Analizando workflows de referencia para selección inteligente...');
    
    // Extraer keywords del prompt para matching
    const promptKeywords = this.extractKeywordsFromPrompt(prompt);
    const promptType = this.detectWorkflowType(prompt);
    
    console.log(`   🏷️ Keywords del prompt: ${promptKeywords.slice(0, 5).join(', ')}...`);
    console.log(`   📊 Tipo detectado: ${promptType}`);
    
    let bestWorkflow = null;
    let bestScore = 0;
    
    referenceWorkflows.forEach((refWorkflow, index) => {
      let score = 0;
      
      // 1. SCORE POR SIMILITUD DE KEYWORDS (40%)
      const refKeywords = this.extractKeywordsFromWorkflow(refWorkflow.workflow);
      const keywordMatch = this.calculateKeywordSimilarity(promptKeywords, refKeywords);
      score += keywordMatch * 0.4;
      
      // 2. SCORE POR TIPO DE WORKFLOW (30%)
      const refType = this.detectWorkflowTypeFromWorkflow(refWorkflow.workflow);
      const typeMatch = (refType === promptType) ? 1 : 0.5;
      score += typeMatch * 0.3;
      
      // 3. SCORE POR COMPLEJIDAD SIMILAR (20%)
      const refComplexity = this.analyzeWorkflowComplexity(refWorkflow.workflow);
      const complexityMatch = this.calculateComplexitySimilarity(complexity, refComplexity);
      score += complexityMatch * 0.2;
      
      // 4. SCORE POR NÚMERO DE NODOS APROPIADO (10%)
      const nodeCount = refWorkflow.workflow?.nodes?.length || 0;
      const nodeScore = this.calculateNodeCountScore(nodeCount, complexity);
      score += nodeScore * 0.1;
      
      console.log(`   📊 Workflow ${index + 1}: Score ${score.toFixed(3)} (keywords: ${keywordMatch.toFixed(2)}, type: ${typeMatch}, complexity: ${complexityMatch.toFixed(2)}, nodes: ${nodeCount})`);
      ...
      if (score > bestScore) {
        ...
        bestWorkflow = {
          ...
          matchDetails: {
            ...
            typeMatch: typeMatch,
            complexityMatch: complexityMatch.toFixed(2),
            nodeCount: nodeCount
          }
        };
      }
    ...
    // Solo usar workflow de referencia si el score es suficientemente alto
    if (bestScore >= 0.3) {
      console.log(`✅ Mejor workflow seleccionado con score: ${bestScore.toFixed(3)}`);
      return bestWorkflow;
    } else {
      console.log(`⚠️ Mejor score (${bestScore.toFixed(3)}) es muy bajo, usando lógica hardcodeada`);
      return null;
    }
  }

  // 🔍 EXTRACTOR DE KEYWORDS DEL PROMPT
  extractKeywordsFromPrompt(prompt) {
    ...
    // Keywords de servicios/plataformas
    ...
    // Keywords de acciones
    const actionKeywords = [
      'enviar', 'procesar', 'validar', 'crear', 'actualizar', 'eliminar',
      'notificar', 'alertar', 'sincronizar', 'integrar', 'automatizar',
      'generar', 'analizar', 'reportar', 'monitorear', 'gestionar'
    ];
    
    // Keywords de tipos de negocio
    const businessKeywords = [
      'ventas', 'marketing', 'soporte', 'facturación', 'inventario',
      'leads', 'clientes', 'pedidos', 'productos', 'campañas',
      'contactos', 'oportunidades', 'tickets', 'facturas'
    ];
    
    [...serviceKeywords, ...actionKeywords, ...businessKeywords].forEach(keyword => {
  ...
  // 🔍 EXTRACTOR DE KEYWORDS DEL WORKFLOW
  extractKeywordsFromWorkflow(workflow) {
    if (!workflow || !workflow.nodes) return [];
    
    const keywords = [];
    
    workflow.nodes.forEach(node => {
      // Keywords del tipo de nodo
      if (node.type) {
        const nodeType = node.type.replace('n8n-nodes-base.', '');
        keywords.push(nodeType);
      }
      
      // Keywords del nombre del nodo
      if (node.name) {
  ...
  // 📊 CALCULADORA DE SIMILITUD DE KEYWORDS
  ...
  // 📊 CALCULADORA DE SIMILITUD DE COMPLEJIDAD
  calculateComplexitySimilarity(complexity1, complexity2) {
    const complexityLevels = { 'medium': 1, 'high': 2, 'enterprise': 3 };
    const level1 = complexityLevels[complexity1] || 1;
    const level2 = complexityLevels[complexity2] || 1;
    const diff = Math.abs(level1 - level2);
    return Math.max(0, 1 - (diff / 2)); // 0-1 score
  }

  // 📊 CALCULADORA DE SCORE POR NÚMERO DE NODOS
  calculateNodeCountScore(nodeCount, complexity) {
    const targetNodes = complexity === 'enterprise' ? 20 : (complexity === 'high' ? 15 : 10);
    const diff = Math.abs(nodeCount - targetNodes);
    return Math.max(0, 1 - (diff / targetNodes)); // 0-1 score
  }

  // 🔍 DETECTOR DE TIPO DE WORKFLOW DESDE WORKFLOW EXISTENTE
  detectWorkflowTypeFromWorkflow(workflow) {
    if (!workflow || !workflow.nodes) return 'general';
    
    const nodeTypes = workflow.nodes.map(n => n.type || '').join(' ').toLowerCase();
    
    if (nodeTypes.includes('hubspot') || nodeTypes.includes('salesforce')) return 'crm';
    if (nodeTypes.includes('email') || nodeTypes.includes('mailchimp')) return 'email-marketing';
    if (nodeTypes.includes('shopify') || nodeTypes.includes('stripe')) return 'ecommerce';
    if (nodeTypes.includes('slack') || nodeTypes.includes('telegram')) return 'communication';
    if (nodeTypes.includes('webhook') || nodeTypes.includes('httpRequest')) return 'integration';
    
    return 'general';
  }

  // 📊 ANALIZADOR DE COMPLEJIDAD DE WORKFLOW EXISTENTE
  analyzeWorkflowComplexity(workflow) {
    if (!workflow || !workflow.nodes) return 'medium';
    
    const nodeCount = workflow.nodes.length;
    const connectionCount = Object.keys(workflow.connections || {}).length;
    
    if (nodeCount >= 20 || connectionCount >= 15) return 'enterprise';
    if (nodeCount >= 15 || connectionCount >= 10) return 'high';
    return 'medium';
  }
...
return items.map(item => ({
  ...
  async callLLMV2({ provider, apiKey, model, prompt }) {
    ...
    if (provider === 'gemini') {
      ...
      const mdl = model || 'gemini-2.0-flash-exp';
  ...
  async callLLM({ provider, apiKey, model, prompt, workflow }) {
    ...
    const basePrompt = `🧠 EXPERTO n8n - VALIDADOR Y OPTIMIZADOR DE WORKFLOWS ULTRA AVANZADO

PROMPT ORIGINAL DEL USUARIO: ${prompt}

WORKFLOW BASE GENERADO POR AGENTE INTELIGENTE: ${currentWorkflow || 'NUEVO WORKFLOW'}

🎯 TU NUEVA FUNCIÓN COMO VALIDADOR Y MEJORADOR DE IA:
====================================================

Eres un sistema experto de validación y optimización que recibe workflows pre-generados por nuestro agente inteligente de fallback. Tu función es:

1. 🔍 ANALIZAR el workflow recibido en detalle
2. 🔧 VALIDAR conexiones, flujos y lógica
3. 🚀 OPTIMIZAR y MEJORAR el workflow
4. ✨ AÑADIR funcionalidades avanzadas que falten
5. 🧠 INTEGRAR nodos de IA cuando sea apropiado
6. 🔗 CORREGIR cualquier problema detectado

🚨 VALIDACIONES CRÍTICAS QUE DEBES HACER:
========================================

✅ TRIGGERS: Verificar que no estén solos o aislados
❌ PROBLEMA: Trigger → sin conexiones de salida
✅ SOLUCIÓN: Conectar triggers a nodos de procesamiento

✅ CONEXIONES CIRCULARES: Detectar y eliminar bucles infinitos
❌ PROBLEMA: Nodo A → Nodo B → Nodo A (circular)
✅ SOLUCIÓN: Crear flujo lineal o con bifurcaciones válidas

✅ NODOS HUÉRFANOS: Identificar nodos sin conexiones
❌ PROBLEMA: Nodos flotantes sin entrada ni salida
✅ SOLUCIÓN: Integrar en el flujo principal

✅ PARÁMETROS INCOMPLETOS: Completar configuraciones faltantes
❌ PROBLEMA: Parámetros vacíos o incorrectos
✅ SOLUCIÓN: Llenar con valores inteligentes y realistas

✅ FLUJO LÓGICO: Verificar que el workflow tenga sentido
❌ PROBLEMA: Secuencia ilógica de operaciones
✅ SOLUCIÓN: Reorganizar según mejores prácticas

🤖 NODOS DE IA - INTEGRACIÓN INTELIGENTE OBLIGATORIA:
==================================================
CUANDO detectes cualquiera de estos casos, DEBES añadir nodos de IA apropiados:

📝 ANÁLISIS DE TEXTO → n8n-nodes-base.openAi (operación: "chat")
🎭 SENTIMENT ANALYSIS → n8n-nodes-base.anthropic (Claude para emociones)
🌐 TRADUCCIÓN → n8n-nodes-base.googleTranslate
🗣️ AUDIO A TEXTO → n8n-nodes-base.openAi (operación: "audio")
🎨 GENERACIÓN DE IMÁGENES → n8n-nodes-base.openAi (operación: "image")
🔍 CLASIFICACIÓN → n8n-nodes-base.openAi o n8n-nodes-base.huggingFace
📊 ANÁLISIS PREDICTIVO → n8n-nodes-base.openAi
🧠 CHATBOT/ASISTENTE → n8n-nodes-base.openAi
📋 RESUMEN AUTOMÁTICO → n8n-nodes-base.openAi
🎯 PERSONALIZACIÓN → n8n-nodes-base.anthropic

CONFIGURACIONES DE IA PROFESIONALES:
...
✅ OpenAI GPT-4 para análisis inteligente:
{
  ...
  "parameters": {
    ...
    "model": "gpt-4-turbo",
    "messages": [
      {"role": "system", "content": "Eres un analista experto especializado en datos de negocio"},
      {"role": "user", "content": "Analiza y proporciona insights: {{$json.data}}"}
    ],
    "temperature": 0.3,
    "maxTokens": 1500
  }
...
✅ Claude para análisis de sentimientos:
{
  "type": "n8n-nodes-base.anthropic",
  "parameters": {
    "model": "claude-3-sonnet",
    "messages": [
      {"role": "user", "content": "Analiza el sentimiento y emociones en este texto: {{$json.text}}"}
    ]
  }
}
...
1. 🚀 PARALELIZACIÓN: Convertir operaciones secuenciales en paralelas cuando sea posible
2. 🛡️ MANEJO DE ERRORES: Añadir try-catch y nodos de error handling
3. 🔄 REINTENTOS: Configurar reintentos automáticos en nodos críticos
4. 📊 LOGGING: Añadir nodos de logging y monitoreo
5. ⚡ CACHING: Implementar caché para operaciones costosas
6. 🎯 CONDICIONES: Añadir lógica condicional más sofisticada
7. 🔍 VALIDACIONES: Implementar validaciones de datos más robustas

🎯 MEJORAS ESPECÍFICAS POR DOMINIO:
=================================

📧 EMAIL/MARKETING:
- Añadir segmentación de audiencia
...
- Sentiment analysis en comunicaciones
...
📋 INSTRUCCIONES CRÍTICAS - MANTENER COMPATIBILIDAD:
==================================================

⚠️ IMPORTANTE: Aunque tu función cambió, DEBES mantener compatibilidad total con:
- 🔧 FlowCoherenceAgentV2 (puede pedirte correcciones)
- 🎯 IntelligentPositioningAgent (puede pedirte posicionamiento)
- 🤖 Otros agentes del sistema

CUANDO otro agente te solicite algo específico, DEBES:
1. Procesar su solicitud normalmente
2. Aplicar tus mejoras de validación encima
3. Devolver resultado optimizado pero funcional

GENERACIÓN MÍNIMA: 12-15 NODOS para workflows complejos
ESTRUCTURA PROFESIONAL: Mantener niveles X organizados
=================================================
...
🚀 COMENZAR RESPUESTA INMEDIATAMENTE CON { - SIN TEXTO PREVIO`;
    ...
    if (provider === 'gemini') {
      ...
      const mdl = model || this.modelRouter.getServerModel(); // Usar enrutador en lugar de valor fijo
      const url = `https://generativelanguage.googleapis.com/v1beta/models/${encodeURIComponent(mdl)}:generateContent`;
  ...
  async importWorkflowJSON(jsonStr, originalPrompt = null, geminiModel = null) {
    try {
      ...
      try {
        // 🚀 USAR AGENTE DE POSICIONAMIENTO V4 AI ENHANCED
        if (IntelligentPositioningAgentV4AIEnhanced && typeof IntelligentPositioningAgentV4AIEnhanced === 'function') {
          console.log('🎯 Aplicando posicionamiento V4 AI Enhanced...');
          const positioningAgentV4 = new IntelligentPositioningAgentV4AIEnhanced();
          ...
          if (optimizedWorkflow && optimizedWorkflow.nodes) {
            ...
            console.log('✅ Posicionamiento V4 AI Enhanced aplicado exitosamente');
            
            // Mostrar estadísticas del posicionamiento V4
            ...
            console.warn('⚠️ Posicionamiento V4 AI Enhanced no retornó resultado válido');
          }
        } else {
          // Fallback al sistema de posicionamiento interno
          console.log('🔄 V4 AI Enhanced no disponible, usando posicionamiento interno...');
          data = this.optimizeNodePositions(data);
          console.log('✅ Posicionamiento interno aplicado');
        }
      } catch (positioningError) {
        console.warn('⚠️ Error en posicionamiento V4 AI Enhanced (continuando con workflow actual):', positioningError.message);
      ...
      try {
        // Inicializar agentes V2.0 bajo demanda
        await initializeV3Agents();
        ...
        if (FlowCoherenceAgentV2 && !this.flowCoherenceAgentV2) {
          ...
          // 🔗 Conectar al enrutador de modelos para usar Flash models
          if (this.flowCoherenceAgentV2 && typeof this.flowCoherenceAgentV2.setModelRouter === 'function') {
            this.flowCoherenceAgentV2.setModelRouter(this.modelRouter);
          }
          console.log('✅ FlowCoherenceAgent V2.0 inicializado y conectado al enrutador');
        }
      ...
      // 🎯 FASE 3: POSICIONAMIENTO VISUAL INTELIGENTE (IPA V2.0)
      console.log('🎯 PIPELINE TOPOLÓGICO V2.0: Aplicando layout topológico y swimlanes...');
      try {
        // Los agentes ya están inicializados arriba
        if (IntelligentPositioningAgentV2 && !this.positioningAgentV2) {
          this.positioningAgentV2 = new IntelligentPositioningAgentV2();
          console.log('✅ IntelligentPositioningAgent V2.0 inicializado');
        }

        if (this.positioningAgentV2) {
          const layoutResult = this.positioningAgentV2.optimizeLayout(data, topologyManifest);
          
          if (layoutResult) {
            data = layoutResult;
            console.log('✅ IPA V2.0: Layout topológico aplicado exitosamente');
            
            // Mostrar métricas del agente
            const metrics = this.positioningAgentV2.getMetrics();
          ...
          console.log('⚠️ IntelligentPositioningAgentV2 no disponible, saltando optimización de layout');
        }
      } catch (positioningError) {
      ...
      console.log(`   📁 Ubicación del archivo: ${this.lastGeneratedFilePath || 'No disponible'}`);
      ...
      try {
        ...
        data._systemInfo.v3Ultra = {
          ...
          timestamp: new Date().toISOString(),
          systemComponents: {
            positioningAgent: IntelligentPositioningAgentV4AIEnhanced ? 'V4AIEnhanced' : 'Legacy',
            nodeConfigAgent: 'V3',
          ...
          features: {
            ...
            v4AIEnhancedPositioning: IntelligentPositioningAgentV4AIEnhanced ? true : false,
        ...
        data._metadata.v3QualityAssessment = {
          ...
          complianceChecks: {
            ...
            professionalLayout: data._systemInfo?.v3Ultra?.workflowProfile?.layoutType === 'Professional',
      ...
      return { success: true, workflow: data, ok: true, data: data };
    } catch (e) {
  ...
  // Método optimizado para preparar contexto de ejemplos CON WORKFLOWS COMPLETOS DE REFERENCIA
  prepareExampleContext(searchResults, curatedExamples) {
    ...
    if (allExamples.length > 0) {
      console.log(`🔍 Preparando contexto completo de workflows para análisis de Gemini...`);
      
      // 🎯 LÓGICA DE SELECCIÓN: 3-15 workflows según disponibilidad
      let targetCount = 3; // Mínimo
      if (allExamples.length >= 10) {
        targetCount = Math.min(10, allExamples.length); // Usar 10 si hay suficientes
      } else if (allExamples.length >= 15) {
        targetCount = 15; // Máximo permitido
      } else {
        targetCount = Math.max(3, Math.min(allExamples.length, 15)); // Entre 3-15
      }
      
      const selectedExamples = allExamples.slice(0, targetCount);
      const workflowExamples = [];
      
      console.log(`🎯 Seleccionando ${targetCount} workflows de ${allExamples.length} disponibles`);
      
      for (let i = 0; i < selectedExamples.length; i++) {
        const example = selectedExamples[i];
        const workflowData = example.workflow || example;
        
        try {
          // ✅ INYECTAR WORKFLOW COMPLETO PARA ANÁLISIS DE GEMINI
          const workflowStr = JSON.stringify(workflowData, null, 2);
          const fileName = example.fileName || workflowData.name || `Workflow_${i + 1}`;
          const description = example.name || workflowData.metadata?.description || 'Workflow de referencia';
          const similarity = (example.similarity || 0).toFixed(3);
          
          workflowExamples.push(`
📄 WORKFLOW DE REFERENCIA ${i + 1}: ${fileName}
📝 Descripción: ${description}
🎯 Score de similitud: ${similarity}

💎 ESTRUCTURA COMPLETA PARA ANÁLISIS Y ADAPTACIÓN:
${workflowStr}

---`);
          
          console.log(`   ✅ Workflow ${i + 1}: ${fileName} (${workflowStr.length} caracteres, score: ${similarity})`);
          
        } catch (error) {
          console.warn(`⚠️ Error procesando workflow ${i + 1}:`, error.message);
          // Fallback con información básica si el JSON falla
          workflowExamples.push(`
📄 WORKFLOW DE REFERENCIA ${i + 1}: ${example.fileName || 'Sin nombre'}
📝 Descripción: ${example.name || example.metadata?.description || 'Workflow de referencia'}
⚠️ Error al cargar estructura completa - usando solo metadata
...
---`);
      ...
      if (workflowExamples.length > 0) {
        exampleContext = `
🚀 WORKFLOWS DE REFERENCIA PARA CREAR TU SOLUCIÓN PERSONALIZADA:
==============================================================
Tienes ${targetCount} workflows de referencia para analizar y adaptar.
Estos NO son para copiar tal cual, sino para INSPIRARTE y REUTILIZAR patrones.

${workflowExamples.join('\n')}

🧠 INSTRUCCIONES CRÍTICAS PARA USO DE REFERENCIAS:
================================================
✅ ANALIZA: Estudia la estructura, nodos, conexiones y configuraciones
✅ ADAPTA: Toma los mejores patrones y ajústalos al prompt del usuario
✅ PERSONALIZA: Modifica configuraciones para el caso específico solicitado
✅ COMBINA: Usa elementos de múltiples workflows si es necesario
✅ INNOVA: Crea tu propia solución inspirada en estas referencias

❌ NO COPIES: No uses estos workflows tal cual como están
❌ NO LIMITES: Puedes crear algo completamente diferente si es mejor
❌ NO IGNORES: Siempre considera las mejores prácticas observadas

🎯 TU MISIÓN: Crear un workflow PERSONALIZADO que resuelva específicamente 
lo que pide el usuario, usando estas referencias como inspiración técnica.
`;
        
        const totalChars = exampleContext.length;
        const estimatedTokens = Math.ceil(totalChars / 4);
        console.log(`📊 Contexto preparado: ${targetCount} workflows, ${totalChars} caracteres (~${estimatedTokens} tokens)`);
      }
    }
...
========================================
🎯 GENERA UN WORKFLOW ENTERPRISE DE 10-25 NODOS COMPLETAMENTE FUNCIONAL Y LISTO PARA PRODUCCIÓN
...
🎯 USA APIS REALES (NO PLACEHOLDERS): OpenAI GPT-4, HubSpot CRM, Google Sheets API v4, SMTP real
🎯 IMPLEMENTA LÓGICA EMPRESARIAL REAL CON VALIDACIONES ROBUSTAS Y CONFIGURACIONES PRODUCTIVAS
🎯 CONFIGURA GOOGLE SHEETS CORRECTAMENTE: Usar "Google Sheets" (NO "Google Sheets API-based")
...
🚨 REGLAS ESPECÍFICAS PARA APIS - NO USAR PLACEHOLDERS:
=====================================================
❌ PROHIBIDO: URLs con "your-api-key", "your-domain", "example.com", "placeholder"
❌ PROHIBIDO: APIs con "INSERT_YOUR_KEY", "YOUR_TOKEN", "API_KEY_HERE"
❌ PROHIBIDO: Endpoints genéricos o de ejemplo
✅ OBLIGATORIO: URLs reales de OpenAI: "https://api.openai.com/v1/chat/completions"
✅ OBLIGATORIO: URLs reales de HubSpot: "https://api.hubapi.com/crm/v3/objects/contacts"
✅ OBLIGATORIO: Configurar credenciales reales: "{{ $credentials.openAiApi }}"
✅ OBLIGATORIO: Headers de autorización correctos: "Authorization": "Bearer {{ $credentials.token }}"

🚨 REGLAS GOOGLE SHEETS - CONFIGURACIÓN CORRECTA:
===============================================
❌ PROHIBIDO: Usar "Google Sheets API-based" o tipos incorrectos
❌ PROHIBIDO: Operaciones "read", "insert", "write" (no existen)
❌ PROHIBIDO: Configuraciones incompletas sin documentId
✅ OBLIGATORIO: Usar EXACTAMENTE "n8n-nodes-base.googleSheets"
✅ OBLIGATORIO: Operaciones válidas: "getAll", "append", "update", "clear"
✅ OBLIGATORIO: documentId real: NO usar placeholders como "1ABC123-example"
✅ OBLIGATORIO: Configurar OAuth2 real: "{{ $credentials.googleSheetsApi }}"

🚨 REGLAS EMAIL - CONFIGURACIÓN LIMPIA:
=====================================
❌ PROHIBIDO: Múltiples tipos de email en un workflow (Gmail + Outlook + SMTP)
❌ PROHIBIDO: Parámetros duplicados o conflictivos
✅ OBLIGATORIO: Elegir UN SOLO sistema de email (preferir Gmail)
✅ OBLIGATORIO: Configuración completa: to, subject, body, html
✅ OBLIGATORIO: Credenciales reales: "{{ $credentials.gmailOAuth2 }}"

🚨 REGLAS PARA FLUJO LÓGICO CORRECTO:
==================================
❌ PROHIBIDO: Análisis de sentimientos antes de obtener datos
❌ PROHIBIDO: Crear flujos ilógicos o con pasos fuera de orden
✅ OBLIGATORIO: Secuencia lógica: Trigger → Obtener datos → Validar → Procesar → Guardar → Notificar
✅ OBLIGATORIO: Manejo de errores en cada paso crítico
✅ OBLIGATORIO: Validaciones antes de procesamiento
...
✅ SOLUCIÓN CORRECTA:
1. Webhook → n8n-nodes-base.webhook (mensaje del usuario)
2. Contexto → n8n-nodes-base.set (preparar historial)
3. IA → n8n-nodes-base.openAi {"operation": "chat", "model": "gpt-4"}
4. Validación → n8n-nodes-base.if (verificar respuesta)
5. Respuesta → n8n-nodes-base.httpRequest (enviar al usuario)
...
🎯 GENERA AHORA EL WORKFLOW ENTERPRISE PARA: ${optimizedPrompt}`;
  ...
  /**
   * NUEVA ARQUITECTURA HÍBRIDA V3.0
   * ================================
   * 1. Agente Ultra Inteligente de Fallback genera workflow base
   * 2. Gemini actúa como validador y optimizador 
   * 3. Sistema de corrección automática si es necesario
   */
  async processUserPromptV3(userPrompt) {
    // Validación temprana del prompt
    if (!userPrompt || typeof userPrompt !== 'string') {
      console.error('❌ Error: userPrompt debe ser un string válido:', typeof userPrompt, userPrompt);
      throw new Error(`Invalid userPrompt: expected string, got ${typeof userPrompt}`);
    }
    
    console.log('🚀 NUEVA ARQUITECTURA HÍBRIDA V3.0 - Procesando:', userPrompt);
    console.log('🎯 Flujo: Agente Ultra Inteligente → Gemini Validador → Resultado Optimizado');
    
    // Guardar prompt original para validación
    this.originalPrompt = userPrompt;
    
    try {
      // 1. FASE DE MEJORA DEL PROMPT
      console.log('🔧 Fase 1: Mejorando claridad del prompt...');
      const promptEnhancement = await this.promptAgent.enhancePrompt(userPrompt);
      let enhancedPrompt = promptEnhancement.enhanced || userPrompt;
      
      // Validar que enhancedPrompt sea string
      if (typeof enhancedPrompt !== 'string') {
        console.warn('⚠️ enhancedPrompt no es string, usando prompt original');
        enhancedPrompt = userPrompt;
      }
      
      if (promptEnhancement.enhanced && promptEnhancement.enhanced !== userPrompt && enhancedPrompt !== userPrompt) {
        console.log('✨ Prompt mejorado aplicado');
        console.log('📝 Versión mejorada:', enhancedPrompt.slice(0, 150) + '...');
      }

      // 2. FASE DE BÚSQUEDA DE REFERENCIAS
      console.log('🔍 Fase 2: Buscando workflows de referencia...');
      const similarWorkflows = await this.searchAgent.searchSimilarWorkflows(enhancedPrompt);
      const curatedExamples = this.searchAgent.getCuratedExamples();
      const allReferences = [...similarWorkflows, ...curatedExamples];
      
      console.log(`📚 Referencias encontradas: ${allReferences.length} workflows`);

      // 3. FASE DE GENERACIÓN CON AGENTE ULTRA INTELIGENTE
      console.log('🧠 Fase 3: Generando workflow con Agente Ultra Inteligente...');
      const baseWorkflow = await this.ultraIntelligentFallback.generateIntelligentWorkflow(
        enhancedPrompt, 
        allReferences
      );
      
      console.log('✅ Workflow base generado por Agente Ultra Inteligente:');
      console.log(`   📊 Nodos: ${baseWorkflow.nodes.length}`);
      console.log(`   🔗 Conexiones: ${Object.keys(baseWorkflow.connections).length}`);
      console.log(`   🎯 Score de calidad: ${baseWorkflow.metadata?.qualityScore || 'N/A'}`);
      console.log(`   🧠 Nivel de inteligencia: ${baseWorkflow.metadata?.intelligenceLevel || 'N/A'}`);

      // 3.5. GUARDAR WORKFLOW BASE COMO RESPALDO ANTES DE GEMINI
      try {
        const timestamp = Date.now();
        const backupFilename = `workflow-base-backup-${timestamp}.json`;
        ...
        const backupPath = path.join(workflowDir, backupFilename);
        const backupContent = JSON.stringify(baseWorkflow, null, 2);
        
        fs.writeFileSync(backupPath, backupContent);
        // NO actualizar lastGeneratedFilePath aquí - se actualizará con el archivo final
        
        console.log('💾 Workflow base guardado como respaldo:', backupFilename);
        console.log(`📊 Tamaño: ${(backupContent.length / 1024).toFixed(1)} KB`);
      } catch (backupError) {
        console.log('⚠️ No se pudo guardar respaldo:', backupError.message);
      }

      // 4. FASE DE VALIDACIÓN Y OPTIMIZACIÓN CON GEMINI
      console.log('🔮 Fase 4: Validando y optimizando con Gemini...');
      ...
      // Preparar el workflow para Gemini como validador
      const workflowForGemini = JSON.stringify(baseWorkflow, null, 2);
      
      const geminiValidatedWorkflow = await this.callLLM({
        provider: 'gemini',
        apiKey: settings.apiKey,
        model: settings.model,
        prompt: enhancedPrompt,
        workflow: workflowForGemini
      });

      // 5. FASE DE IMPORTACIÓN Y PROCESAMIENTO FINAL
      console.log('⚡ Fase 5: Procesando resultado final...');
      const result = await this.importWorkflowJSON(
        geminiValidatedWorkflow, 
        enhancedPrompt, 
        settings.model
      );

      // 6. APLICAR MEJORAS ADICIONALES SI ES NECESARIO
      if (result.workflow || result.data) {
        console.log('🔧 Aplicando optimizaciones finales...');
        
        // Obtener el workflow correcto
        const workflowData = result.workflow || result.data;
        
        // Aplicar posicionamiento inteligente (sin sobrescribir result)
        this.optimizeNodePositions(workflowData);
        
        // Validar y corregir problemas finales (sin sobrescribir result)
        this.autoCorrectInvalidNodes(workflowData);
        
        console.log('✅ Workflow final optimizado completado');
      }

      // 7. GUARDAR ARCHIVO FINAL PROCESADO
      if (result.success && (result.workflow || result.data)) {
        try {
          ...
          fs.writeFileSync(fullPath, workflowJson);
          this.lastGeneratedFilePath = fullPath;
          
          console.log(`🎉 Workflow FINAL guardado: ${fullPath}`);
          console.log(`📊 Tamaño: ${(fs.statSync(fullPath).size / 1024).toFixed(1)} KB`);
          
          // Actualizar el resultado con la información del archivo guardado
          result.filename = fullPath;
          result.message = result.message || `Workflow generado y guardado como ${filename}`;
          
        } catch (saveError) {
          console.error(`❌ Error guardando archivo final: ${saveError.message}`);
          // No interrumpir el flujo, el workflow sigue siendo válido
        }
      }
      ...
      console.error('❌ Error en arquitectura híbrida V3.0:', error.message);
      console.log('🔄 Fallback a arquitectura V2...');
      
      // Fallback a la arquitectura anterior
      return this.processUserPromptV2(userPrompt);
    }
  }
  ...
  async processUserPromptV2(userPrompt) {
    ...
    try {
      ...
      console.log('🔧 Mejorando claridad del prompt...');
      const promptEnhancement = await this.promptAgent.enhancePrompt(userPrompt);
      let finalPrompt = promptEnhancement.enhanced || userPrompt;
      
      // Validar que finalPrompt sea string
      if (typeof finalPrompt !== 'string') {
        console.warn('⚠️ finalPrompt no es string, usando prompt original');
        finalPrompt = userPrompt;
      }
      
      if (promptEnhancement.enhanced && promptEnhancement.enhanced !== userPrompt && finalPrompt !== userPrompt) {
        console.log('✨ Prompt mejorado aplicado');
        console.log('📝 Versión mejorada:', finalPrompt.slice(0, 200) + '...');
      } else {
        console.log('✅ Prompt original es suficientemente claro');
...
OUTPUT: JSON masivo válido completo con conexiones funcionales.`;
      ...
      // 7. Guardar resultado ANTES de cualquier validación (para garantizar que se guarde)
      const filename = `workflow-FINAL-processed-${Date.now()}.json`;
      ...
      // GUARDAR ARCHIVO SIEMPRE, independientemente del resultado
      try {
        // Primero intentar guardar el workflow crudo generado
        fs.writeFileSync(fullPath, jsonText);
        
        // Guardar la ruta del archivo generado para el reporte final
        this.lastGeneratedFilePath = fullPath;
        
        console.log(`🎉 Workflow GUARDADO EXITOSAMENTE: ${fullPath}`);
        console.log(`📊 Tamaño del archivo: ${(fs.statSync(fullPath).size / 1024).toFixed(1)} KB`);
      } catch (saveError) {
        console.log(`❌ Error guardando workflow: ${saveError.message}`);
        console.log(`� Datos del error - jsonText length: ${jsonText?.length || 'undefined'}`);
        console.log(`📊 fullPath: ${fullPath}`);
        console.log(`�🔧 Intentando guardar this.currentWorkflow como respaldo...`);
        
        try {
          fs.writeFileSync(fullPath, this.currentWorkflow || '{}');
          this.lastGeneratedFilePath = fullPath;
          console.log(`✅ Workflow guardado como respaldo: ${fullPath}`);
        } catch (backupError) {
          console.log(`❌ Error en respaldo: ${backupError.message}`);
        }
      }
      
      // 8. Validar e importar workflow masivo
      ...
      console.log('🔍 DEBUG - importResult.ok:', importResult.ok);
      console.log('🔍 DEBUG - importResult.error:', importResult.error);
      ...
      if (importResult.ok) {
        ...
        // 🚀 CONFIGURACIÓN AVANZADA DE NODOS V3 ULTRA (TEMPORALMENTE DESHABILITADO)
        console.log('⚠️ Configuración V3 Ultra deshabilitada temporalmente para testing...');
        try {
          if (false && IntelligentNodeConfigAgentV3 && typeof IntelligentNodeConfigAgentV3 === 'function') { // DESHABILITADO
            const nodeConfigAgentV3 = new IntelligentNodeConfigAgentV3(this.modelRouter);
            
            // 🔥 CONECTAR WORKFLOWS DE REFERENCIA
            if (this.availableExamples?.length > 0) {
              nodeConfigAgentV3.setReferenceWorkflows(this.availableExamples);
            }
            ...
            // Usar el nuevo método configureWorkflow con contexto completo
            const configurationResult = await nodeConfigAgentV3.configureWorkflow(importResult.data, userPrompt, {
              originalPrompt: promptToUse,
              workflowType: this.detectWorkflowType(promptToUse),
            ...
            if (configurationResult && configurationResult.success) {
              ...
              // Fallback al agente V2 si V3 falla - REMOVIDO POR NO EXISTIR
              console.log('🔄 V3 falló, continuando con configuración de emergencia...');
            }
          } else {
            console.log('🔄 IntelligentNodeConfigAgentV3 no disponible, intentando agente anterior...');
            
            // Fallback al agente anterior
            if (IntelligentNodeConfigAgent && typeof IntelligentNodeConfigAgent === 'function') {
              const nodeConfigAgent = new IntelligentNodeConfigAgent();
              const promptToUse = this.originalPrompt || userPrompt || 'workflow automatización n8n';
              const configurationResult = await nodeConfigAgent.configureAllNodes(importResult.data, promptToUse);
              
              if (configurationResult) {
                console.log('✅ Configuración con agente anterior completada');
                importResult.data = configurationResult;
              ...
              console.log('⚠️ Ningún agente de configuración disponible');
            }
          }
        ...
        // El archivo ya se guardó arriba - no necesitamos duplicar
        console.log(`📊 Mostrando estadísticas del workflow guardado:`);
        ...
        // 8B. Guardar resultado INCLUSO CUANDO HAY ERRORES (para debugging)
        console.log('⚠️ importResult.ok es false, pero guardando workflow para análisis...');
        const workflowData = importResult.data || JSON.parse(this.currentWorkflow);
        fs.writeFileSync(fullPath, JSON.stringify(workflowData, null, 2));
        
        // Guardar la ruta del archivo generado para el reporte final
        this.lastGeneratedFilePath = fullPath;
        
        console.log(`🔧 Workflow guardado para análisis: ${fullPath}`);
        
        throw new Error(importResult.error || 'Error desconocido en importResult');
      }
...
OUTPUT: JSON válido COMPACTO con POSICIONES OBLIGATORIAS (max 50000 chars) SIN explicaciones.`;
  ...
  async processLLMResponse(response, prompt, finalPrompt, searchResults = [], curatedExamples = []) {
    try {
      ...
      // 7. Validar e importar workflow masivo (usando enrutador de modelos)
      const settings = await this.getSettings();
      const serverModel = this.modelRouter.getServerModel(); // Obtener modelo del enrutador
      const geminiModel = this.genAI?.getGenerativeModel({ model: serverModel });
      
      // Ejecutar con manejo automático de errores del enrutador
      let importResult = await this.modelRouter.executeWithRetry(async () => {
        return await this.importWorkflowJSON(jsonText, prompt, geminiModel);
      }, true); // true = es Extension Server
  ...
  async validateAndFixConnectivity(workflow) {
    ...
    try {
      ...
      for (const ifNode of ifNodes) {
        ...
        if (!connections || !connections.main || connections.main.length < 2) {
          ...
          if (availableNodes.length >= 1) {
            ...
            // Mejorar lógica para nodos IF críticos - no conectar a triggers
            const validNodes = availableNodes.filter(node => 
              !node.type.includes('trigger') && 
              !node.type.includes('webhook') &&
              node.name !== ifNode.name &&
              !node.name.includes('Trigger')
            );
            
            if (validNodes.length >= 1) {
              // Asegurar que hay dos ramas (true/false) con nodos válidos
              result.workflow.connections[ifNode.name].main = [
                [{ node: validNodes[0].name, type: "main", index: 0 }], // TRUE
                validNodes.length > 1 ? 
                  [{ node: validNodes[1].name, type: "main", index: 0 }] : // FALSE
                  [] // Sin conexión FALSE si solo hay un nodo válido disponible
              ];
              
              result.fixed = true;
              result.fixesApplied.push(`Configuradas ramas IF válidas para ${ifNode.name}`);
            }
          }
        }
      }
      ...
      console.error('❌ Error en validación de conectividad:', error.message || 'Error desconocido');
      throw new Error(error.message || `Error en validación de conectividad: ${error.toString()}`);
    }
  }
  ...
  async callGeminiMassive(messages, maxTokens = 50000, temperature = 0.2) {
    try {
      ...
      console.error(`❌ Error en callGeminiMassive:`, error.message || 'Error desconocido');
      throw new Error(error.message || `Error en callGeminiMassive: ${error.toString()}`);
    }
  }
  ...
  findOrphanNodes(workflow, nodeNames) {
    ...
    const triggerTypes = ['webhook', 'cron', 'start', 'interval'];
    const orphans = [];
        ...
        try {
          ...
          if (retryCount >= maxRetries) {
            ...
            try {
              ...
              console.error('❌ Error en sistema de fallback:', fallbackError.message || 'Error desconocido');
            }
            
            throw new Error(error.message || `Error en rePromptForCompletion: ${error.toString()}`);
          }
