/**
 * AGENTE DE CONFIGURACIÓN DE NODOS AVANZADO - ES MODULE VERSION
 * 
...
export class IntelligentNodeConfigAgent {
    constructor() {
    ...
    analyzeDataFlow(workflow) {
        ...
        // Identificar puntos de entrada (nodos sin conexiones de entrada)
        workflow.nodes.forEach(node => {
            const hasIncomingConnections = Object.values(workflow.connections || {}).some(nodeConnections => 
                Object.values(nodeConnections || {}).some(connections => 
                    connections.some(conn => conn.node === node.name)
                )
            );
            
            if (!hasIncomingConnections) {
                dataFlow.entryPoints.push(node);
            }
        });
    ...
    /**
     * Construye el prompt masivo para Gemini
     */
...
- **Prompt Original**: ${context.originalPrompt}
- **Total de Nodos**: ${context.totalNodes}
- **Dominio de Negocio**: ${context.businessDomain}
- **Tipos de Nodos**: ${Object.keys(context.nodeTypes).join(', ')}
...
- **IF**: conditions con Value1, Operation, Value2 claros
- **Switch**: mode "rules", rules con output routing
- **Merge**: mode "append" o "combine", clashHandling para duplicados
- **Condiciones**: usar expresiones {{ $json.field }} comparison operators
...
¡CADA NODO DEBE ESTAR LISTO PARA USAR INMEDIATAMENTE TRAS CONFIGURAR CREDENCIALES!
`;
    }
