...
// 🚀 INTEGRACIÓN DEL ROUTER DE MODELOS GEMINI
...
class N8nAIAssistant {
  constructor() {
    ...
    this.promptAgent = new PromptEnhancementAgentOptimized();
...
OUTPUT: JSON válido COMPACTO con POSICIONES OBLIGATORIAS (max 50000 chars) SIN explicaciones.`;
  ...
  // MÉTODO PARA LLAMADAS MASIVAS A GEMINI - MODO PRODUCCIÓN CON ROUTER
  async callGeminiMassive(messages, maxTokens = 50000, temperature = 1.0) {
    try {
      console.log(`🚀 Llamada masiva a Gemini con ${messages.length} mensaje(s) usando Router Inteligente`);
      
      // Preparar prompt para el router
      const promptText = messages.map(msg => 
        typeof msg === 'string' ? msg : (msg.content || msg.text || JSON.stringify(msg))
      ).join(' ');
      
      console.log('🎯 Usando Gemini Model Router para Extension Server (modelos Pro)');
      
      // Llamar al router con configuración específica para extension server
      const routerResult = await this.geminiRouter.generateContent(
        promptText,
        'extension-server', // Nombre del agente para usar modelos Pro
        {
      ...
      console.log(`✅ Router exitoso con ${routerResult.modelName} en intento ${routerResult.attemptNumber}`);
      console.log(`� Costo estimado: $${routerResult.cost.toFixed(4)} USD`);
      console.log(`⏱️ Tiempo de respuesta: ${routerResult.responseTime}ms`);
      
      // Registrar llamada en el tracker global
      GeminiCallTracker.recordCall('MainGenerator', 'generateContent', promptText.length, routerResult.modelUsed);
      
      return routerResult.content;
      
    } catch (error) {
      console.error(`❌ Error en Router Gemini:`, error.message);
      
      // Fallback a método directo en caso de error crítico del router
      console.log('🔄 Activando fallback directo a Gemini...');
      return await this.callGeminiDirect(messages, maxTokens, temperature);
    }
  }

  // MÉTODO DE FALLBACK DIRECTO (mantenido para emergencias)
  async callGeminiDirect(messages, maxTokens = 50000, temperature = 1.0) {
    try {
      console.log(`� Fallback directo a Gemini 2.5 Pro`);
      ...
      const response = await fetchModule(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.5-pro:generateContent?key=${process.env.GEMINI_API_KEY}`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      console.log('📊 GEMINI FALLBACK - Respuesta recibida:');
      console.log(`   📏 Longitud: ${text.length.toLocaleString()} caracteres`);
