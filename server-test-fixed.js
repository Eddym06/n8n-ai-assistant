// Servidor simplificado para tests - Puerto 3000 (CommonJS)
...
app.post('/api/agents/analysis', (req, res) => {
  res.json({
    success: true,
    agent: 'error-analysis',
    result: 'Analysis completed',
    recommendations: ['Optimize node 1', 'Add error handling']
  });
});

app.post('/api/agents/performance', (req, res) => {
  res.json({
    success: true,
    agent: 'performance-optimization',
    optimizations: ['Cache results', 'Parallel execution'],
    estimatedImprovement: '25%'
  });
});

app.post('/api/agents/custom-node', (req, res) => {
  res.json({
    success: true,
...
app.post('/api/agents/routing', (req, res) => {
  res.json({
    success: true,
    agent: 'multi-agent-router',
    route: 'analysis -> optimization -> generation',
    status: 'completed'
  });
});
...
// Error handling para tests específicos
...
let requestCount = 0;
app.use((req, res, next) => {
  if (req.path.includes('/rate-limit-test')) {
    requestCount++;
    if (requestCount > 100) {
