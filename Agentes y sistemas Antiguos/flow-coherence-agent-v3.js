/**
 * FlowCoherenceAgent V3.0 - Optimizado para eficiencia API
...
const { apiOptimizer } = require('./api-optimization-manager');
...
class FlowCoherenceAgentV3 {
    ...
    performLocalValidation(workflow) {
        ...
        ).slice(0, 3); // Limita a 3 para evitar spam
    ...
    async processWorkflow(workflow, options = {}) {
        ...
        try {
            ...
            remainingIssues = remainingIssues
                ...
                .slice(0, 2); // Máximo 2 problemas por API call
...
PROBLEMAS: ${JSON.stringify(issues.slice(0, 2))} // Max 2 problemas

WORKFLOW: ${nodeCount} nodos, ${Object.keys(workflow.connections || {}).length} conexiones

Responde en JSON con MÁXIMO 2 acciones:
{
  "actions": [
    {
      "type": "fix_critical",
    ...
    applyCorrectionPlan(workflow, plan) {
        ...
        plan.actions.slice(0, 2).forEach(action => { // Max 2 acciones
            try {
    ...
    getEfficiencyStats() {
        ...
        return {
            ...
            efficiency: {
                localResolutionRate: this.callCount > 0 ? 0.8 : 1.0, // Estimado
                avgProcessingTime: '< 100ms local',
...
module.exports = FlowCoherenceAgentV3;
