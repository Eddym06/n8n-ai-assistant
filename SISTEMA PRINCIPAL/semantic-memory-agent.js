...
class SemanticMemoryAgent {
  constructor(options = {}) {
    ...
    // Inicializar Gemini AI con modelo Flash
    try {
      if (process.env.GEMINI_API_KEY) {
        this.genAI = new GoogleGenerativeAI(process.env.GEMINI_API_KEY);
        // Usar modelo Flash inicial, pero el enrutador lo cambiará automáticamente
        this.model = this.genAI.getGenerativeModel({ model: "gemini-2.5-flash" });
        console.log('🤖 SemanticMemoryAgent inicializado con Gemini Flash');
      } else {
        console.log('⚠️ Gemini AI no disponible para SemanticMemoryAgent');
      }
    } catch (error) {
      console.log('⚠️ Gemini AI no disponible para SemanticMemoryAgent');
    }
  ...
  setModelRouter(modelRouter) {
    ...
    console.log('🔗 SemanticMemoryAgent conectado al enrutador de modelos');
  }
  ...
  getCurrentModel() {
    if (this.modelRouter) {
      ...
      if (this.genAI && this.model && agentModel !== this.currentModelName) {
        ...
        console.log(`🔄 SemanticMemoryAgent actualizó a: ${agentModel}`);
      }
      return agentModel;
    }
