...
class PromptEnhancementAgent {
  constructor(options = {}) {
    ...
    try {
      if (process.env.GEMINI_API_KEY) {
        ...
        console.log('🤖 PromptEnhancementAgent inicializado con Gemini Flash');
      } else {
  ...
  setModelRouter(modelRouter) {
    ...
    console.log('🔗 PromptEnhancementAgent conectado al enrutador de modelos');
  }
  ...
  getCurrentModel() {
    if (this.modelRouter) {
      ...
      if (this.genAI && this.model && agentModel !== this.currentModelName) {
        ...
        console.log(`🔄 PromptEnhancementAgent actualizó a: ${agentModel}`);
      }
      return agentModel;
    }
  ...
  analyzePromptComplexity(prompt) {
    ...
    return {
      ...
      analysis: {
        ...
        hasTechnicalTerms: intermediateScore > 0.2,
        hasAdvancedSpecs: advancedScore > 0.2,
  ...
  async adaptivePromptProcessing(prompt) {
    ...
    switch (complexity.enhancementNeeded) {
      case 'FULL':
        ...
        // REMOVIDO: this.enhancePrompt(processedPrompt) - evita recursión infinita
        processedPrompt = this.autoCorrectInvalidOperations(processedPrompt).correctedPrompt || processedPrompt;
        break;
  ...
  translateVaguePromptToTechnical(prompt) {
    ...
    // Validar que prompt sea una string
    if (!prompt || typeof prompt !== 'string') {
      console.log('⚠️ Prompt inválido, usando prompt vacío');
      prompt = '';
    }
...
1. **ARQUITECTURA DE FLUJO COMPLETA:**
   - Nodo trigger específico (webhook, start, cron)
...
Este workflow debe funcionar de manera completamente automática, procesando las conversaciones de clientes y gestionando todo el flujo desde la recepción inicial hasta la confirmación final, asegurando que ninguna solicitud se pierda y que tanto el equipo como el cliente estén informados apropiadamente.`;
  ...
  validatePrompt(prompt) {
    ...
    // Length validation
    if (length < minLength) {
      warnings.push('Prompt too short');
      score -= 20;
    ...
    // Content validation
    if (prompt.includes('n8n') || prompt.includes('workflow') || prompt.includes('automation')) {
      score += 15; // Contains relevant keywords
    }

    // Technical terms
    if (/\b(webhook|api|database|email|integration)\b/i.test(prompt)) {
