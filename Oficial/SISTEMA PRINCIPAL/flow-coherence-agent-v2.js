...
export default class FlowCoherenceAgentV2 {
  constructor(apiKey) {
    ...
    // Usar modelo Flash inicial, pero el enrutador lo cambiará automáticamente
    this.model = this.genAI.getGenerativeModel({ model: 'gemini-2.5-flash' });
    
    // Referencia al enrutador de modelos (se asignará desde Extension Server)
    this.modelRouter = null;
    this.currentModelName = 'gemini-2.5-flash'; // Track current model
    
    // Cache para análisis repetidos
    this.analysisCache = new Map();
    
    console.log('🔧 FlowCoherenceAgent v2.0 inicializado con Gemini Flash');
  }
  ...
  setModelRouter(modelRouter) {
    ...
    console.log('🔗 FlowCoherenceAgent conectado al enrutador de modelos');
  }
  ...
  getCurrentModel() {
    if (this.modelRouter) {
      ...
      if (this.genAI && this.model && agentModel !== this.currentModelName) {
        ...
        console.log(`🔄 FlowCoherenceAgent actualizó a: ${agentModel}`);
      }
      return agentModel;
    }
  ...
  async processWorkflow(workflow, originalPrompt, topologyManifest = null) {
    ...
    // 🚀 OPTIMIZACIÓN: Procesar en lotes inteligentes para workflows masivos
    const subflowsArray = Array.from(manifest.subWorkflows.entries());
    const batchSize = manifest.complexity === 'MASSIVE' ? Math.min(20, Math.max(5, Math.floor(subflowsArray.length / 10))) : 
                     manifest.complexity === 'LARGE' ? 10 : 
                     manifest.complexity === 'MEDIUM' ? 5 : 1;
    
    console.log(`📦 Usando procesamiento en lotes: ${batchSize} sub-flujos por lote`);

    // Procesar por lotes
    for (let i = 0; i < subflowsArray.length; i += batchSize) {
      const batch = subflowsArray.slice(i, i + batchSize);
      console.log(`--- Procesando lote ${Math.floor(i/batchSize) + 1}/${Math.ceil(subflowsArray.length/batchSize)}: ${batch.length} sub-flujos ---`);
      
      const batchProblems = [];
      const batchWorkflows = [];
      
      // Recopilar problemas del lote
      for (const [rootNodeName, subflowData] of batch) {
      ...
      // Si hay problemas en el lote, procesarlos en una sola llamada
      if (batchProblems.length > 0) {
        console.log(`🔍 Detectados problemas en ${batchProblems.length}/${batch.length} sub-flujos del lote`);
        
        try {
          // Obtener plan de corrección para todo el lote
          const batchPlan = await this.getBatchCorrectionPlan(batchProblems, originalPrompt, manifest.complexity);
          
          if (batchPlan && batchPlan.corrections && batchPlan.corrections.length > 0) {
            console.log(`📋 Plan de corrección en lote recibido: ${batchPlan.corrections.length} acciones`);
            
            // Aplicar correcciones al workflow principal
            finalWorkflow = this.applyCorrections(finalWorkflow, batchPlan);
            allChanges.push(...batchPlan.corrections);
            totalProblemsFixed += batchProblems.reduce((sum, bp) => sum + bp.problems.totalProblems, 0);
            
            console.log(`✅ Lote procesado exitosamente`);
          }
        } catch (error) {
          console.warn(`⚠️ Error procesando lote:`, error.message);
          
          // Fallback: procesar individualmente solo los problemáticos
          for (const batchProblem of batchProblems.slice(0, 3)) { // Máximo 3 para evitar spam
            try {
              const contextualPrompt = this.generateContextualPrompt(
                batchProblem.subflowData, 
                originalPrompt, 
                manifest.complexity
              );
              const plan = await this.getCorrectionPlan(batchProblem.tempWorkflow, batchProblem.problems, contextualPrompt);
              ...
              if (plan && plan.corrections && plan.corrections.length > 0) {
                ...
                totalProblemsFixed += batchProblem.problems.totalProblems;
              }
            } catch (individualError) {
              console.warn(`⚠️ Error individual en ${batchProblem.rootNodeName}:`, individualError.message);
            }
          }
        }
      }
...
`;
  ...
  /**
   * 🚀 OPTIMIZACIÓN: Plan de corrección para múltiples sub-flujos en lote
   */
  async getBatchCorrectionPlan(batchProblems, originalPrompt, complexity) {
    // Registrar llamada en el tracker
    GeminiCallTracker.recordCall('FlowCoherenceAgent', 'getBatchCorrectionPlan');

    // Preparar resumen del lote
    const batchSummary = batchProblems.map(bp => ({
      name: bp.rootNodeName,
      nodeCount: bp.subflowData.nodeCount,
      problems: bp.problems.totalProblems,
      mainIssues: [
        ...bp.problems.orphanNodes.map(n => `Nodo huérfano: ${n}`),
        ...bp.problems.incompleteIFs.map(n => `IF incompleto: ${n}`),
        ...bp.problems.disconnectedNodes.map(n => `Desconectado: ${n}`)
      ].slice(0, 3) // Máximo 3 problemas por sub-flujo para no saturar
    }));

    const prompt = `
Eres un experto en automatización empresarial con n8n. Analiza este LOTE de sub-flujos y proporciona correcciones eficientes.

CONTEXTO ORIGINAL: ${originalPrompt.substring(0, 200)}...
COMPLEJIDAD: ${complexity}
LOTE A PROCESAR: ${batchProblems.length} sub-flujos

RESUMEN DEL LOTE:
${JSON.stringify(batchSummary, null, 2)}

REGLAS DE CORRECCIÓN EN LOTE:

🔗 EFICIENCIA MASIVA:
- Para ${complexity} workflows, ser ultra-permisivo
- Solo corregir errores críticos de conectividad
- Ignorar nodos huérfanos si son parte de módulos independientes
- Permitir arquitecturas paralelas complejas

🔀 PATRONES EMPRESARIALES:
- Los módulos pueden ser independientes (webhooks separados)
- Los IF deben tener al menos salida 'main'
- Los Merge consolidar múltiples entradas
- Flujos paralelos son normales y válidos

⚡ TOLERANCIA ALTA:
- Workflows >100 nodos requieren máxima tolerancia
- Enfocarse solo en conectividad crítica
- Permitir sub-flujos completamente independientes

RESPONDE SOLO las correcciones más críticas en formato JSON:
{
  "corrections": [
    {
      "type": "connect_nodes" | "add_condition" | "fix_merge",
      "source": "nombre_nodo_origen",
      "target": "nombre_nodo_destino", 
      "outputType": "main" | "else",
      "reason": "razón_breve"
    }
  ],
  "summary": "resumen_cambios_críticos_aplicados"
}
`;
    ...
    try {
      ...
      console.error('❌ Error obteniendo plan de corrección en lote:', error);
      return { corrections: [], summary: 'Error en generación de plan en lote' };
    }
  }
