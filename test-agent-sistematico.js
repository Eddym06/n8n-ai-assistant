...
class AgentTester {
    ...
    mockSemanticAnalysis(prompt) {
        return {
            ...
            userIntent: ['webhook_receive', 'data_validation', 'payment_processing'],
            complexity: 'medium',
            entities: ['order', 'customer', 'payment'],
            requiredIntegrations: ['shopify', 'mysql', 'stripe'],
            dataPoints: ['order_data', 'customer_info', 'payment_status'],
            processFlows: ['order_intake', 'validation', 'payment', 'fulfillment'],
            aiNodesNeeded: ['fraud_detection', 'recommendation_engine'],
            automationPatterns: ['webhook_response', 'data_transformation'],
            validationRequirements: ['schema_validation', 'business_rules'],
            semanticKeywords: ['order', 'payment', 'validation'],
            actionWords: ['process', 'validate', 'transform'],
            businessTerms: ['e-commerce', 'shopify', 'stripe']
        };
    }
    ...
    async testSemanticAnalysis() {
        ...
        try {
            ...
            const analysis = await this.agent.performDeepSemanticAnalysis(prompt);
            
            console.log('✅ Análisis semántico exitoso');
            console.log(`   🎯 Dominio: ${analysis.businessDomain}`);
            console.log(`   📊 Complejidad: ${analysis.complexity}`);
            console.log(`   🎪 Intenciones: ${analysis.userIntent.length}`);
    ...
    async testNodeGeneration() {
        ...
        try {
            ...
            const nodes = await this.agent.generateIntelligentNodes(plan, semanticAnalysis);
    ...
    async testConnectionGeneration() {
        ...
        try {
            ...
            const connections = this.agent.createIntelligentConnections(nodes, plan);
